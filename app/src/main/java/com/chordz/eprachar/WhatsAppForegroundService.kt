...
class WhatsAppForegroundService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        try {
            when (intent?.action) {
                ACTION_SEND_WEBHOOK -> {
                    val phoneNumber =
                        intent.getStringExtra(EXTRA_PHONE_NUMBER) ?: return START_STICKY
                    val callStatus = intent.getStringExtra(EXTRA_CALL_STATUS) ?: "missed"

                    Log.d(
                        "WhatsAppService",
                        "Received webhook task: $phoneNumber, status: $callStatus"
                    )
    ...
    private fun processWebhook(phoneNumber: String, callStatus: String) {
        ...
        val userId =
            AppPreferences.getLongValueFromSharedPreferences(AppPreferences.ADMIN_NUMBER).toString()
        ...
        val json = JSONObject().apply {
            when (user.lowercase()) {
                "true craft" -> {
                    ...
                    url = "https://nonrecurently-diverse-deedee.ngrok-free.dev/webhook/aimessage"
                }
